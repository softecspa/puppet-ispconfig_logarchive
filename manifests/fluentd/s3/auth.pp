define ispconfig_logarchive::fluentd::s3::auth (
  $custom_config,
) {
  ...
  $s3_settings = {
    ...
    'path'        => "logs/${cluster}/${::hostname}/auth/%Y/%m/%d/"
  }

  $default_config = merge ($ispconfig_logarchive::params::auth_fluentd_output_defaults,$s3_settings)
  $config         = merge ($default_config,$custom_config)

  fluentd::match{'auth_main':
    configfile  => 'auth',
    type        => 's3',
    pattern     => 's3.auth.*',
    config      => $config,
