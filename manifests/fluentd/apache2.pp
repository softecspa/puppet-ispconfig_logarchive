class ispconfig_logarchive::fluentd::apache2 {
  ...
  fluentd::source { 'apache_main':
    configfile  => 'apache2',
    type        => 'tail',
    format      => '/^(?<vhost>[^|]*)\|\|\|\|(?<size>[0-9]*)\|\|\|\|(?<client>[0-9\.]*) (?<host>[^ ]*) (?<vhost>[^ ]*) (?<user>[^ ]*) \[(?<time>[^\]]*)\] "(?<method>\S+)(?: +(?<path>[^ ]*) +\S*)?" (?<code>[^ ]*) (?<size>[0-9]*)(?: "(?<referer>[^\"]*)" "(?<agent>[^\"]*)")? "(?<cookie>[^\"]*)" (?<serv_time_second>[0-9]*)/(?<serv_time_microsecond>[0-9]*) (?<conn_status>[X|\+|\-]?) (?<process_id>[0-9]*)$/',
